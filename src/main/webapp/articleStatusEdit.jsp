...
<div class="container" style="width: 300px;">
    <%Article article = (Article)session.getAttribute("article");
        UserRole role = UserSession.getUser().getRole();
        ArrayList<User> users = ( ArrayList<User>)session.getAttribute("users");%>

    <h2 class="form-signin-heading">
        Current status: <%out.print(article.getStatus().toString());%>
    </h2>

    <div class="container" style="width: 300px;">
        <form action="/changeStatus?status=DELETED&id=<%out.print(article.getId());%>" method="post">
    ...
    <div class="container" style="width: 300px;">
        <form action="/changeStatus?status=CORRECTOR&id=<%out.print(article.getId());%>" method="post">
            <select name="newUser">
                <%for(User user: users){%>
    ...
    <div class="container" style="width: 300px;">
        <form action="/changeStatus?status=EDITOR&id=<%out.print(article.getId());%>" method="post">
            <button id = "btnEditor" class="btn btn-lg btn-primary btn-block" type="submit">Appoint editor</button>
        </form>
    </div>

    <div class="container" style="width: 300px;">
        <form action="/changeStatus?status=AUTHOR&id=<%out.print(article.getId());%>" method="post">
    ...
    <div class="container" style="width: 300px;">
        <form action="/changeStatus?status=WEBSITE&id=<%out.print(article.getId());%>" method="post">
